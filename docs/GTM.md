...
> **버전**: `0.1.1` · 최종 갱신: 2026-06-27(ADR-0010 의도축으로 §2 재구조화) · 상태: Living Document
...
### 1.1 한 문장 포지셔닝
> "검색 엔진은 링크를 주고, **cerebro는 연결된 지도를 준다** — 사용자가 원하는 모든 공개 주제를 위한 인터랙티브 3D 지식 그래프."
...
## 2. 타깃: 사용 의도(의도축) — 선험적 비치헤드 베팅 금지

> **관점 정렬([ADR-0010](./adr/0010-bm-broad-first-intent-axis.md) · [PRD §2](./PRD.md))**: 제품은 주제를 가리지 않고(broad), 타깃은 "사람 세그먼트"가 아니라 **사용 의도(활용 관점)** 로 본다. **어느 의도를 핵심 니치로 좁힐지는 미리 베팅하지 않고 Phase 0 계측(§6.1)으로 발견**한다. (이전 "B2B 리서처 비치헤드" 가설은 데이터의 공인 수렴 + broad-first 결정으로 폐기.)

| 의도(관점) | 핵심 Job-to-be-Done | 도달 채널(예시) | 수익화 후보(가설) |
|---|---|---|---|
| **투자** | 대상의 호재/악재·리스크 판단 | 투자 커뮤니티, 뉴스레터 | 모니터링·심층(높음) |
| **취업·커리어** | 지원/면접 전 다각도 파악 | 취업 카페·디스코드, 대학 커뮤니티 | 심층 리포트 |
| **콘텐츠** | 출처 붙은 배경·사실 수집 + 시각자료 | 유튜브/뉴스레터, X | **바이럴 엔진**(결과물 공유)·임베드 |
| 경제/사회/쇼핑/건강/관계/도서 | 맥락별 활용 | 오가닉·검색 | TBD(데이터로) |

> **집중 원칙(YAGNI)**: 초기 **획득 메시지**는 입소문·검색량이 큰 의도(취업·콘텐츠·투자)부터 시드하되, 이는 *마케팅 채널 선택*일 뿐 **제품/수익 니치를 미리 고정하는 것이 아니다**. 니치 확정은 §6.1 계측 결과로(ADR-0010).
...
- **온드 미디어**: "기업 리서치 5분 단축법", "면접 전 기업 분석 체크리스트" 등 고관여 의도(취업·콘텐츠·투자) 키워드 블로그 → CTA로 cerebro 검색 유도.
...
## 6.1 Phase 0 계측 — 의도축 니치 발견 (M1~M2)

> 목적: 어느 "활용 관점(의도)"이 리텐션·지불의향이 높은지를 **데이터로** 찾아 수익을 좁힌다([ADR-0010](./adr/0010-bm-broad-first-intent-axis.md)). 익명·무PII로 수집(§6 AARRR과 연결).

- **검색 단위**: 주제 유형(기업/공인/제품/작품/기타)·카테고리.
- **관점(angle) 노드 단위(9종)**: 노출 → 열림(click) → 체류(dwell) → 재방문 → 공유 → 저장 → (M3)모니터링 신청.
- **도출 지표**:
  - **(주제유형 × 관점) 히트맵** — 어느 셀이 저장·모니터링·재방문을 지배하나(가설: 기업×투자/취업).
  - **한도 도달 이벤트** — 어떤 캡(검색·LLM·저장·내보내기)을 가장 많이 치나 → 지불의향 위치.
  - **캐시 적중률 + 검색당 LLM 비용** → 마진 가시화([ADR-0011](./adr/0011-llm-report-cache-and-licensing-gate.md)).
- **도구**: PostHog/Plausible 무료 티어 + Supabase 이벤트 테이블(익명).
- 이 데이터가 "어느 의도 열을 Pro 딥팩으로 깊게 팔지"와 전환 트리거([ROADMAP §4.1](./ROADMAP.md))를 동시에 결정한다.
...
## 7.2 프로슈머 무료/유료 경계 (M3 1단계 상세)

> **원칙: 게이팅은 생산·지속·대량 측(creator/power)에만, 소비(viewer) 측은 전부 무료** → 바이럴 루프 보존. 근거 = [ADR-0010](./adr/0010-bm-broad-first-intent-axis.md), 비용·캐시 = [ADR-0011](./adr/0011-llm-report-cache-and-licensing-gate.md).

| 기능 | 무료 | Pro | 선 긋는 원칙 |
|---|---|---|---|
| 키워드 검색(익명·무로그인) | ✅ 시간당 15·일 50/IP | ✅ 무제한(공정사용) | 바이럴 진입점 — 한도는 어뷰즈·쿼터 방어용이지 수익 레버 아님 |
| 3D 탐색·노드 상세(출처·신뢰도) | ✅ | ✅ | 3D는 후킹·공유 대상 → 100% 무료 |
| **활용 관점 리포트(LLM)** | ✅ fresh 일 5건/세션·IP + 캐시 무제한(초과=휴리스틱) | ✅ 무제한 + 심층 모드 | 핵심 레버 = 무료는 폭(맛보기), Pro는 깊이. 비용 천장 방어선 |
| 공유: 이미지·`/g/:slug`·임베드 | ✅ 워터마크 有 | ✅ 워터마크 제거 | 무료 공유물 = 광고(바이럴). 제거만 과금(Canva/Datawrapper 패턴) |
| 공유받은 그래프 열람 | ✅ 전체(무로그인) | ✅ | 신규 방문자가 풀 가치를 봐야 전환 — 소비 측 게이팅 금지 |
| **저장 보드** | ✅ 로그인 시 10개 | ✅ 무제한 + 폴더·태그 | 하이브리드 계정(저장=로그인). 워크플로 해자 체험은 무료, 확장은 Pro |
| 내보내기 PNG·PDF | △ 워터마크 PNG | ✅ 고해상 PNG·PDF | 업무·콘텐츠 산출물 = 지불의향 명확 |
| **모니터링/알림** | ❌ | ✅ | 일회성 검색 → 지속 의도 전환. **의도축 레버의 정점** |
| 비교 뷰(여러 주제 병렬) | ❌ | ✅ | 경쟁·다중 분석 = 업무 가치 |

- **무료 한도(₩0 인프라 방어)**: 익명 검색 시간당 15·일 50/IP · LLM fresh 일 5건/세션 · 글로벌 서킷 $8/월([ADR-0011](./adr/0011-llm-report-cache-and-licensing-gate.md)) · 저장 10개. 저장 보드는 스니펫 영구저장 금지·라이브 재조회([DATA-SOURCING §11](./DATA-SOURCING.md)).
- **의도축 연결**: 모니터링/알림이 의도축 정점 레버(일회성→지속 의도). *어느 관점*이 저장·모니터링·재방문을 유발하는지가 "깊게 팔 의도 열"을 알려준다(§6.1).
...
| **Pro** | 헤비 리서처·크리에이터 | 베타 ₩6,900 → 정상 ₩9,900 · 연 ₩89,000 (가설) | 상한 대폭↑, LLM 무제한·심층, 저장/내보내기, 워터마크 제거, 모니터링/알림 | 개인 SaaS 심리적 가격대(₩1만 미만). 베타 평생고정(초기 한정)=입소문, 정상가=마진. 경계 상세 §7.2 |
