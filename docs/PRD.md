...
> **담당 역할**: Project Owner
> **버전**: `0.1.1` · 최종 갱신: 2026-06-27(ADR-0010 BM 방향에 정렬) · 상태: Living Document
...
어떤 주제(기업·브랜드·제품·작품·공인 등 공개 정보가 있는 무엇이든)를 검색하면, 웹 곳곳에 흩어진 공개 정보를 cerebro가 수집·정제해 **중심(검색어)을 둘러싼 3D 구체 노드**의 마인드맵으로 즉시 보여준다. 사용자는 텍스트를 읽는 대신 **공간을 탐색하며** 대상의 전체 그림과 관계 구조를 직관적으로 파악한다.
...
### 1.3 한 문장 포지셔닝
> "검색 엔진은 링크를 주고, cerebro는 **연결된 지도**를 준다 — 사용자가 원하는 모든 공개 주제를 위한 인터랙티브 3D 지식 그래프."
>
> **BM 방향([ADR-0010](./adr/0010-bm-broad-first-intent-axis.md))**: 제품은 주제를 가리지 않고(broad) 열어두고, 수익화·니치는 "활용 관점(의도)축"에서 데이터로 발견한다 — 상세는 §2 및 [GTM](./GTM.md).

---

## 2. 사용자 & 사용 의도 (의도축)

> **관점 전환([ADR-0010](./adr/0010-bm-broad-first-intent-axis.md))**: cerebro는 **주제를 가리지 않는다**(broad). 사용자는 *무엇을 검색하느냐(주제)*가 아니라 ***왜 검색하느냐(의도)*** 로 갈린다. 같은 주제("삼성전자", "손흥민")라도 의도가 다르면 원하는 것이 다르다. 그래서 타깃을 "사람 유형(페르소나)"이 아니라 **사용 의도(활용 관점)** 로 정의하고, 수익화·니치는 이 **의도축**에서 데이터로 발견한다(선험적으로 한 의도/세그먼트에 베팅하지 않음).

**대상 주제**: 기업·브랜드·제품·작품·공인 등 **공개 정보가 있는 주제 전반**. 개인은 **공개정보/공인 한정**(PIPA 가드레일 + 데이터 특성상 자연 수렴 — 일반인은 거의 결과가 나오지 않음). 주제 범위를 좁히지 않는 것이 바이럴 표면을 키운다.

### 2.1 사용 의도 아키타입 (= 활용 관점 리포트 9종과 정렬)

각 의도는 LLM 활용 관점 리포트([ADR-0008](./adr/0008-llm-usage-report.md))의 관점 노드로 직접 대응된다. 아래는 **예시이자 측정 대상**이며, *어느 의도가 핵심 니치인지는 Phase 0 계측([GTM §6.1](./GTM.md))으로 결정*한다.

| 의도(관점) | 핵심 Job-to-be-Done | 예시 맥락(사람) | 수익화 후보 신호(가설) |
|---|---|---|---|
| **투자** | 대상의 호재/악재·리스크를 빠르게 판단 | 투자 전 기업·코인·인물 점검 | 모니터링·심층(높음) |
| **취업·커리어** | 지원/면접 전 대상을 다각도로 파악 | 취준·이직(구 "민호") | 심층 리포트 |
| **콘텐츠** | 대상 배경·사실을 출처와 함께 빠르게 수집 | 기자·크리에이터(구 "세라") | 임베드·내보내기·바이럴 |
| **경제·산업** 등 (사회/쇼핑/건강/관계/도서) | 맥락별 활용 | 일반 탐색·B2B 리서치(구 "지윤") | TBD(데이터로) |

> 기존 페르소나(지윤=B2B 리서치, 민호=취업, 세라=콘텐츠)는 위 의도의 **사람 예시**로 흡수된다. 단 "주 타깃/비치헤드"라는 **선험적 우선순위는 두지 않는다** — broad로 열고 의도축에서 발견한다.

### 2.2 공통 가치 (의도 불문)
- **흩어진 공개정보를 한 번의 검색으로 통합** → 탭 10개의 수고 제거.
- **출처·수집시각·신뢰도가 붙은 노드** → 사실 확인·인용 가능(AI 챗봇의 환각 대비 차별점).
- **중심-가지 3D 구조 + 활용 관점 리포트** → "전체 그림 + 이 정보를 내 의도에 어떻게 쓸지"를 한 화면에.
...
- **노드 상세 패널**: 요약 · 출처(URL/이름) · 수집시각 · 신뢰도 · **활용 관점 리포트**(관점별 활용법 — 의도축·핵심 차별점, ADR-0008/0010).
...
- 회원가입/로그인/개인화·저장 보드(MVP는 익명 단발 검색). → 저장 보드·계정은 **하이브리드(검색 익명·저장 로그인)로 M2/M3에 BM과 함께** 도입(ADR-0010).
- 다국어(en/ja) — i18n **구조만** 초기 반영, 실제 번역은 추후.
- 결제·유료 플랜·API 판매(BM 확장은 GTM에서 별도 — 프로슈머 freemium·가격·경계는 ADR-0010 · GTM §7.2/§8).
...
- 노드 클릭 시 패널 오픈. 필수 필드:
  ...
  | 활용 관점 리포트 | 관점별(투자·취업·콘텐츠 등) 활용 리포트 — **의도축·핵심 차별점**. 단문이 아닌 리포트 본문(ADR-0008/0010) |
...
>
> **의도축 지표(BM)**: 어느 활용 관점(의도)이 리텐션·지불의향이 높은지 — *주제유형 × 관점 히트맵*이 니치 발견의 핵심. 상세 계측은 [GTM §6.1](./GTM.md) Phase 0([ADR-0010](./adr/0010-bm-broad-first-intent-axis.md)).
