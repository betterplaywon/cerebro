...
## 11. 유료화 전 상업적 이용 라이선스 게이트 (M3 전 필수)

> 현재(무료 토이)는 합법 영역에 있다(공식 API · 개인정보 비저장 · 30분 갱신캐시 · 전 출처표기 · 대상이 공인으로 수렴). 아래는 **과금(M3) 전** 클리어할 **운영 위생 체크리스트**(법률 자문 아님). 근거: 약관 리서치 + [ADR-0011](./adr/0011-llm-report-cache-and-licensing-gate.md).

| 소스 | 상태 | 과금 전 액션 |
|---|---|---|
| **네이버** 검색 API | 🟠 오너 확인 게이트 | `developers.naver.com` 약관 원문(접근 차단 → 오너 직접 확인): (a) 영리/상업 이용 가부 (b) 스니펫 저장·DB화·재배포 제한 (c) 출처표기 의무 (d) 유료 노출 시 제휴 필요. **추정으로 과금 시작 금지.** |
| **카카오** 검색 API | 🟠 저장보드 재설계 조건부 | 이용약관 제11조3항·운영정책 제5조30호(사전승낙 없는 복제·장기보관·제3자 제공 제한), 제5조20호(캐시는 UX·최신유지만). → 저장 보드는 스니펫 영구저장 금지·라이브 재조회(ADR-0011). 대량 지속저장 필요 시 카카오 사전승낙(데브톡). |
| **위키백과** CC BY-SA 4.0 | 🟢 조건부 통과 | 상업 OK. 단 출처 + "Wikipedia, CC BY-SA 4.0" 표기 유지, 위키 파생·LLM 요약 2차물에 독점 저작권 주장 금지(보수적 SA 인정), 위키 파생 콘텐츠에 DRM 미적용. |

**공통 게이트**: ① 쿼터 헤드룸 — 네이버 25k·카카오 검색 합산 50k/일(쿼리당 네이버 5콜·카카오 3콜 곱셈 주의) → 캐시 적중률·쿼터 카운터·증설(제휴) 경로 확보. ② 전 결과 출처표기 일관성(유료 화면 포함). ③ 결제·약관·환불·세금 + PIPA 삭제경로([BACKLOG](./BACKLOG.md) NOW#2) 완료 — 유료는 PIPA 책임 가중.
