...
## 11. 소스 라이선스 분리 — 무료 표시(Layer A) vs 수익화·재가공(Layer B)

> 네이버 오픈API FAQ·약관(오너 직접 확인, 2025-05-27 개정)으로 **네이버·카카오 검색결과는 상업 이용·재가공·장기저장 불가**가 확정됨 → 소스를 라이선스 기준 2개 레이어로 분리한다. 근거·결정: [**ADR-0014**](./adr/0014-source-license-segmentation.md)(+ [ADR-0011](./adr/0011-llm-report-cache-and-licensing-gate.md)). (법률 자문 아님 — 운영 위생 기준.)

| 레이어 | 소스 | 허용 | 금지 |
|---|---|---|---|
| **A — 무료 표시·단순캐시** | **네이버·카카오** 검색 API | 출처 표기해 **독립 노드로 표시**(약관 2.1 독립적 노출·무수정) + **부하용 단순 캐시 ≤30분** | **재가공(LLM 입력)·장기저장(7일 캐시·저장보드·모니터링 지문)·수익화(유료/광고)** |
| **B — 수익화·재가공·저장·모니터링** | **위키백과**(CC BY-SA) 앵커 + 공공데이터포털 + 앱스토어(iTunes) + 유튜브(Data API) + **상업용 웹검색(Tavily 유료 등)** | LLM 활용 리포트·7일 리포트 캐시·저장 보드·모니터링 = **이 레이어 위에서만** | 위키: 출처 + "Wikipedia, CC BY-SA 4.0" 표기·2차물 독점 저작권 주장·DRM 금지 |

- **파이프라인 규칙**: 수집 단계에서 소스를 **"표시(A)"와 "분석/저장(B)"으로 라벨링**. **무료 LLM 리포트도 Layer B 입력만** 사용(무료/유료 일관). 네이버/카카오는 어떤 경우에도 LLM 입력·7일 캐시·저장·모니터링 대상에서 제외.
- **트레이드오프**: 무료 리포트의 한국어/시의성 깊이가 약화됨 → Layer B에 상업용 웹검색(Tavily 유료)·공공데이터 보강([ADR-0005](./adr/0005-defer-broad-web-search.md) 부분 해제 트리거).

**공통 게이트**: ① 쿼터 헤드룸 — 네이버 25k·카카오 검색 합산 50k/일(쿼리당 네이버 5콜·카카오 3콜 곱셈 주의) → 캐시 적중률·쿼터 카운터·증설 경로 확보. ② 전 결과 출처표기 일관성(유료 화면 포함). ③ 결제·약관·환불·세금 + PIPA 삭제경로([BACKLOG](./BACKLOG.md) NOW#2) 완료 — 유료는 PIPA 책임 가중.
