...
3. **LLM 예산 자동 상한(서킷 브레이커)** — 현재 유일 방어가 '수동 키 제거' → 카운터/임계치로 자동 차단(자체 사용량 기록 포함).
