...
> · **활용 관점 동적 생성(ADR-0019·PR #71)**: 카테고리(활용 관점)가 한정적이라는 피드백 → 고정 9개 enum(투자/취업/경제/사회/건강/관계/쇼핑/도서/콘텐츠) 강제를 제거하고 **LLM이 주제·출처에 맞춰 관점을 동적 생성**(주제 밀착 라벨 6~9개). 가드(투자 면책·PIPA·출력측 PII 재마스킹)·shared/web 무변경. **효과는 `ANTHROPIC_API_KEY` 설정 시 LLM 경로에서만.**
...
- **활용 관점 리포트(ADR-0008·0019)**: 키(`ANTHROPIC_API_KEY`) 설정 시 검색당 1회 Claude(Sonnet 4.6) 호출 →
  중심 노드=핵심 정보 요약, 자식 `usage` 노드=관점별 상세 리포트. **관점은 고정 목록이 아니라 주제·출처에
  맞춰 LLM이 동적 생성**(ADR-0019 — 예: nvidia면 "AI·데이터센터"·"GPU 기술"·"경쟁 구도"·"수출 규제"…).
  상세 패널이 `report`를 문단으로 렌더. **키 없으면 휴리스틱(카테고리/토픽)
  그래프로 폴백(지출 0)**. 캐시(30분)로 동일 쿼리 재요청 추가 지출 0.
  - **리포트 입력 소스 = Layer B만**(위키·공공데이터 등 상업 OK). 네이버·카카오(Layer A)는 약관상
    재가공·저장 금지로 제외(ADR-0014). **단, `PERSONAL_USE_MODE=true`(개인 전용·비공개·비영리)면
    Layer A도 포함**해 '처음처럼' 한국어/시의성 깊이 복원(ADR-0018, 기본 OFF·공개 시 반드시 OFF).
