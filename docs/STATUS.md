...
apps/web (Vite+React+R3F)  검색 UI · 세레브로 로더(CSS) · MindMapCanvas(lazy) · DetailPanel · api/client
```
...
- **여전히 403**: 더 기다림. 1시간+ 지나도면 Console **결제 계정 연결**만 확인(무료 100/일은 과금 없음).
- (값 노출 금지: 키는 env에서 읽어 쓰되 출력하지 말 것.)

## 5. 다음 작업 백로그 (우선순위)
1. **구글 3-소스 검증 마무리** (위 §4).
2. **출처 표시 UX**(무키·안전): 검색 결과에 "분석된 출처 N건(위키·네이버·구글)" 요약 + 출처 타입 한글 배지. "정보의 출처 표시" 요구 충족.
