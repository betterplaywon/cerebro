...
**[pipa-sensitive-filter-hardening] PIPA/민감정보 필터 고도화 (구조적 식별자 패턴 확장 + 오탐 저감)**

판단 기준 1~5위를 모두 충족하는 최적 단일 진입점이다. (1)가치: PIPA는 프로젝트 골든룰이고 코드로 실제 누출을 확인했다 — pii.ts는 주민번호(성별코드 [1-4])와 휴대전화 2종만 마스킹하고, 외국인등록번호(성별코드 5~8)·이메일·카드번호는 그대로 통과한다(UGC 유입 시 노출). (2)착수: readiness=ready, 외부 키/승인/트래픽 의존 0. (3)노력대비효과: 핵심이 단일 파일(pii.ts) 정규식 확장+테스트라 S에 근접(M 하단), Luhn 오탐 저감만 추가 검증. (4)의존성: 선결 후보 없음. (5)비용/리스크: $0·신규 의존성 0이며, snippet을 Claude로 보내기 전 PII까지 줄여 예산($8.8)에도 미세 우호적. 다른 'now' 후보(삭제요청 경로=BE+FE+문서로 더 큼, 예산 auto-cap=실트래픽/배포 전이라 긴급도 다소 낮음)보다 첫 커밋으로 끝맺기 가장 깔끔하다.
...
| 티어 | 항목 (우선순위 순) |
|---|---|
| **NOW** | 1. pipa-sensitive-filter-hardening · 2. data-deletion-request-flow · 3. llm-budget-auto-cap |
...
_M1 출시를 막는 하드 게이트(PIPA 골든룰·M1 Exit) + 라이브 예산 안전을 먼저 닫는다. 셋 모두 readiness=ready, $0, 외부 의존 0이라 다음 세션이 즉시 끝낼 수 있고, 코드 검증으로 실제 공백을 확인했다(pii.ts 외국인등록번호·이메일 미마스킹, registry 차단목록 부재, report.ts res.usage 폐기·수동 키제거만이 유일한 예산 방어)._

### 1. PIPA/민감정보 필터 고도화 (구조적 식별자 패턴 확장 + 오탐 저감)
`id: pipa-sensitive-filter-hardening` · 노력 **M(중간)** · 🟢 착수 가능 · tier:next
...
### 2. 삭제(잊힐 권리) 요청 대응 경로 — 수집 차단목록 + 제품 내 요청 채널/고지 + 운영 런북
...
### 3. LLM 예산 자동 상한(서킷 브레이커): 누적 사용량 추적 + 임계치 도달 시 자동 폴백
`id: llm-budget-auto-cap` · 노력 **M(중간)** · 🟢 착수 가능 · tier:next
