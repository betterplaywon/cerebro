...
/** 출력 토큰 상한. 관점 수↑(최대 16개)에 따른 JSON 절단→파싱실패 폴백을 막기 위해 상향(ADR-0019). */
const MAX_TOKENS = 8000;
/** 서로 구별되는 관점 목표 상한(build.ts MAX_USAGE_BRANCHES와 정합). 오너 요청으로 9→16 상향. */
const MAX_ANGLES = 16;
...
const SYSTEM_PROMPT = [
  ...
  `   - 서로 뚜렷이 구별되는 관점을 최대 ${MAX_ANGLES}개까지 만든다. 출처가 풍부한 주체는 세부 축까지 폭넓게 펼쳐 10개 이상으로 풍성하게, 빈약하면 적게. 겹치거나 사실상 같은 관점은 하나로 합친다.`,
  '   - 여러 축을 폭넓게 아우른다. 예: ' +
    ANGLE_EXAMPLES.join(', ') +
